...
static int akm8963_init_flag =0; // 0<==>OK -1 <==> fail
...
//Begin: Added by l00184147 20120223 
static short calibration_value=0;
//End: Added by l00184147 20120223 

/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id akm8963_i2c_id[] = {{AKM8963_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_akm8963={ I2C_BOARD_INFO("akm8963", (0x1c>>1))};
...
static int AKECS_CheckDevice(void)
{
	...
	AKMDBG("ret = %d",ret);
	sprintf(id, "compass8963=0x%d", buffer[0]);
	//set_id_value(COMPASS_ID, id);
	if(ret < 0)
	{
		return ret;
	}
...
//Begin: Added by l00184147 20120223 
	short flag;
//End: Added by l00184147 20120223 
	AKMDBG("1 akm8963_unlocked_ioctl  invalid argument cmd=0x%x\n\n",cmd);
	switch (cmd)
	{
...
//End: Added by l00184147 20120223 
		...
		case ECS_IOCTL_GET_LAYOUT:
			AKMDBG("ECS_IOCTL_GET_LAYOUT\n");
			layout[0] = 2;
			layout[1] = atomic_read(&data->layout);
			layout[2] = 0;
			AKMDBG("ECS_IOCTL_GET_LAYOUT  layout[1] =%d\n",layout[1]);
...
static int __init akm8963_init(void)
{
	...
	i2c_register_board_info(3, &i2c_akm8963, 1);
	return 0;    
}
/*----------------------------------------------------------------------------*/
